...
const OrangeCapStats = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/batting_stats")
        ...
        <table className="table text-white rounded-xl overflow-hidden shadow-lg border-separate border-spacing-y-1">
          <thead className="bg-slate-950">
            <tr className="border-b border-white/20">
              ...
              <th className="text-white">Runs Scored</th>
              <th className="text-white">Matches</th>
              <th className="text-white">Innings</th>
              <th className="text-white">Highest Score</th>
            ...
            {filteredPlayers.map(player => {
              ...
              return (
                ...
                >
                  ...
                  <td className="text-white">{player.RunsScored}</td>
                  <td className="text-white">{player.Matches}</td>
                  <td className="text-white">{player.BatInnings}</td>
                  <td className="text-white">{player.HighestScore}</td>
                  <td className="text-white">{player.BatAvg}</td>
                  <td className="text-white">{player.BatSR}</td>
                </tr>
              );
            })}
          </tbody>
